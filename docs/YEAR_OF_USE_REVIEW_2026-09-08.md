...
### 5. 給油は記録できるのに、振り返る場所が無かった 〔直した〕

`docs/HABIT_DESIGN.md` は給油を「唯一の月単位の接点」と位置づけている。
なのに記録の**入口しかなく**、保存した直後に燃費が1回出るだけだった。
1年で75件たまるものが、どこからも見えない。

`FuelHistoryScreen` を足した。出すのは4つ。

```
 回数        14回
 平均燃費    14.2 km/L    ← 満タン法。継ぎ足しは次の満タンの量に足す
 給油代      ¥95,060
 1kmあたり   ¥13.1        ← 維持費の実感にいちばん近い
```

1行ごとにその回の燃費を添え、満タンでない回は「継ぎ足し」と書く
（燃費が出ない理由がその場で分かる）。計算は純関数
`FuelEfficiency.history` / `FuelSummary.of` に置いてテストで固定した。
見え方は `test/golden/goldens/screen_fuel_history.png`。

車両詳細の「給油を記録」は「給油と燃費」に変え、この一覧を開くようにした。

### 6. この1年のふりかえりが、いちばん奥にあった 〔直した〕

`YearInReviewScreen` は車両詳細をスクロールした先にあった。**1年ぶんが
溜まった人にいちばん見せたい画面が、いちばん遠い。** ホームの
「メンテナンスの記録」に出している1年の数字をタップで開くようにした。
記録は押されたときに読む（ホームを開くたびに40件超を読まない）。

### 7. 直したついでに見つけた小さな壊れ方 〔直した〕
...
- 給油の**推移（グラフ）**は出していない。数字と一覧だけ。季節で燃費が
  変わることは、並べただけでは読み取りにくい
- ホームの「たびの記録」は集計を1年で切っている。**それ以前の記録は
  数字に入らない**（一覧には出る）
...
```
 flutter test --exclude-tags "emulator || golden"   4,441件 全パス
