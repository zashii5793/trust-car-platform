...
- 1フリート = 100台 × 整備履歴 各50件 = **5,000件/フリート**、10フリートで **50,000件** 規模。
- **Lazy migration が特に有利**: フリート管理画面は「1車両の履歴を1ページずつ」読むため、
  移行は**アクセスされたドキュメントだけ**で済み、全50,000件を一括変換する必要がない。
- **backfill する場合**のコスト目安: 50,000件の書き直し ≒ 50,000 write。Blaze で概ね
  $0.09/10万write のため **1テナントあたり $0.05 未満**。テナント単位（`companyId`/`shopId`）で
  区切って夜間バッチ実行すれば、稼働影響とコストを制御できる。
- 負荷・移行の検証は `scripts/seed_load_test.js`（B2Bフリート既定）で再現可能。
