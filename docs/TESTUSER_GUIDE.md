...
画面下の `プロフィール` → `サポート・法的情報` → `ご意見・不具合の報告`

ご利用の端末とアプリのバージョンは自動で添えられるので、
**気づいたことだけ書いていただければ大丈夫です。**

> アプリのバージョンは `プロフィール` 画面のいちばん下にも出ています
> （例: `バージョン 1.0.0 (a1b2c3d)`）。メールや電話でご連絡いただく場合は、
> この行をそのままお伝えいただけると、こちらで再現しやすくなります。
