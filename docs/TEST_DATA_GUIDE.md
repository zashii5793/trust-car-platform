...
正しい順番で全部のシードを流し、最後に会話ログの書き出しと
`verify_personas.js`（ルール越しの確認43件）まで走ります。
...
### seed_year_of_use.js が投入するもの（629ドキュメント）
...
| アクセサリーのクチコミ | 4 | 直近1年 |
| 車両の走行距離更新（merge） | 4 | — |
| 店舗とのチャット（inquiries） | 8 スレッド | 直近1年 |
| チャットの往復（inquiries/messages） | 26 | 直近1年 |
| 法人B（20台）の整備記録 | 50 | 直近1年 |
...
#### 1年ぶんのやりとり（seed_year_of_use.js）

ペルソナ A と **タカヤモーター**（一部は品川）の 8 スレッド・26 通。上の 4 件が
「機能が動く」ことの確認用なのに対し、こちらは **1年つき合った状態**（溜まって
並ぶ・古いものは閉じている・直近だけ未読）を見るためのもの。会話の中身は
整備記録（`maintenance_records`）と噛み合わせてあるので、突き合わせても辻褄が合う。

| スレッド | 時期 | 状態 |
|---|---|---|
| ハイエースのオイル交換 | 約10か月前 | 完了（4往復） |
| ロードスターのタイヤ交換見積もり | 約4.5か月前 | 完了（5往復） |
| ハイエースの車検見積もり | 約3か月前 | 完了（6往復・明細の受け渡しあり） |
| アルファードのバッテリー相談 | 約2か月前 | 完了（3往復） |
| リース満了前の点検（品川） | 約1.5か月前 | 完了（2往復） |
| エアコンフィルター交換 | 約1.3か月前 | 完了（3往復） |
| ハイエースのオイル交換（直近） | 約1か月前 | 返信あり・**ユーザー側未読1件** |
| ロードスターの異音相談 | 3日前 | **店舗が未返信**（店側の「未対応」確認用） |

**店側からも同じ会話を開けます**（下の「店舗ペルソナ」参照）。

### 店舗ペルソナ（店側のログイン）

```
メール     : shop.owner@example.com
パスワード : password123
```

⚠️ **店の文書IDと店主の uid は必ず同じにしてください。**
アプリは `ShopService.getMyShop(uid)` で `shops/{uid}` を直接引き、
`firestore.rules` も `request.auth.uid == shopId` で当事者を判定します。
この2つがずれていると、店主でログインしても自分の店が見つからず、
**問い合わせ一覧もチャットもルールに弾かれて常に空**になります。
（2026-09-20 まで uid が `shop-owner-takaya` でずれており、店側の画面は
一度も実データで開けていませんでした。`seed_shop_owner.js` で修正済み。）
...
## 3.5 シードが本当に「見える」かを確かめる

```bash
node scripts/verify_personas.js
```

各ペルソナで **Auth エミュレータに実際にログインし**、アプリが投げるのと同じ形の
クエリを **本物の `firestore.rules` 越しに**流します。NG が1件でもあれば終了コード 1。

なぜ要るか: Dart のペルソナテスト（`test/integration/persona_*.dart`）は
`FakeFirebaseFirestore` で動き、**ルールを評価しません**。そのため
「テストは緑なのに本番では1件も返らない」クエリを原理的に検出できません。
2026-09-08 のレビューで 14 件見つかったのは、すべてこの型の不具合でした。

確認しているもの（43 件・ペルソナ A〜J と店舗）:

- A の1年ぶんのデータ（車両・ドライブログ・給油・整備・クチコミ）が読めるか
- 店舗とのチャットが **お客様側と店側の両方から**同じ会話として開けるか
- 未読バッジ・未対応の絞り込み・集計クエリ（`count()`）がルールを通るか
- **他人の会話・他人の整備記録が読めないこと**（読めてしまったら重大）
- ルールに合わない形のクエリが、意図どおり拒否されること
...
node scripts/seed_year_of_use.js --delete --emulator       # year_of_use_v1 を削除
```

`seed_year_of_use.js --delete` は問い合わせの `messages` サブコレクションも
一緒に消します（親だけ消すとメッセージが孤児として残り、流し直したときに
古い会話が混ざります）。
