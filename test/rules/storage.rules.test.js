...
describe('shops/{shopId}/caseStudies/{fileName} — 施工事例画像', () => {
  const SHOP_UID = 'shop_owner_789';
  const otherUid = 'customer_abc';
  const shopPath = `shops/${SHOP_UID}/caseStudies/before_001.jpg`;

  describe('write（アップロード）', () => {
    test('ショップオーナーは自店舗のパスに画像を書き込める', async () => {
      await assertSucceeds(
        uploadBytes(ref(storageFor(SHOP_UID), shopPath), PNG_BYTES, PNG_META),
      );
    });

    test('他ユーザーはショップの施工事例画像を書き込めない', async () => {
      await assertFails(
        uploadBytes(ref(storageFor(otherUid), shopPath), PNG_BYTES, PNG_META),
      );
    });

    test('未認証ユーザーは書き込めない', async () => {
      await assertFails(
        uploadBytes(ref(unauthStorage(), shopPath), PNG_BYTES, PNG_META),
      );
    });

    test('非画像ファイルは書き込めない（isImageFile）', async () => {
      await assertFails(
        uploadBytes(ref(storageFor(SHOP_UID), shopPath), new Uint8Array([1, 2, 3]), {
          contentType: 'application/pdf',
        }),
      );
    });

    test('5MB超の画像は書き込めない（isValidFileSize）', async () => {
      const big = new Uint8Array(5 * 1024 * 1024 + 1);
      await assertFails(
        uploadBytes(ref(storageFor(SHOP_UID), shopPath), big, PNG_META),
      );
    });
  });
  ...
  describe('read（閲覧）', () => {
    ...
    test('オーナー自身は閲覧できる', async () => {
      await assertSucceeds(getDownloadURL(ref(storageFor(SHOP_UID), shopPath)));
    });

    test('認証済み他ユーザーも閲覧できる（公開プロフィール情報）', async () => {
      await assertSucceeds(getDownloadURL(ref(storageFor(otherUid), shopPath)));
    });

    test('未認証ユーザーは閲覧できない', async () => {
      await assertFails(getDownloadURL(ref(unauthStorage(), shopPath)));
    });
  });
  ...
  describe('delete（削除）', () => {
    ...
    test('オーナーは自店舗の画像を削除できる', async () => {
      await assertSucceeds(deleteObject(ref(storageFor(SHOP_UID), shopPath)));
    });

    test('他ユーザーは削除できない', async () => {
      await assertFails(deleteObject(ref(storageFor(otherUid), shopPath)));
    });

    test('未認証ユーザーは削除できない', async () => {
      await assertFails(deleteObject(ref(unauthStorage(), shopPath)));
    });
  });
});
