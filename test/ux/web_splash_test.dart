...
void main() {
  ...
  group('Web の起動スプラッシュ', () {
    ...
    test('取得が始まったら、終わるまで時間で見切らない', () {
      // 2026-08-25 実測: 同じビルドでもフォントの到着は 3.4 秒のときと
      // 9.8 秒のときがあった。「最初のフレームから N 秒」で見切ると、
      // 遅い回で消してしまい豆腐が出る。fetch を包んで取得の開始を掴み、
      // 終わるまで待つ。
      expect(html, contains('originalFetch'));
