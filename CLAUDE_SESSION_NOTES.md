...
## 2026-06-21 価格表示ポリシー（確定）

### 原則：価格は「面」で分ける
| 面 | 性質 | 価格表示 |
|---|---|---|
| A. 公開の店舗比較（マーケットプレイス） | 工場 vs 工場の横並び | ❌ 出さない。値下げ競争（タイヤ等の仕入れ規模勝負）を招き、サービスの厚い工場が不利になる |
| B. ユーザー私的な整備履歴（maintenance_records） | 本人のみ閲覧 | ⭕ 「なぜ高いか」を本人が理解する用途。比較ではなく納得 |

- 公開面（A）での価格ランキング/横並び比較機能は**作らない**。工場が競うのはレビュー・専門性・サービス内容。
- 価格・内訳は **B面（ユーザー所有・ルールで非公開）に閉じる**。既存アーキ（maintenance_records がユーザー所有）と一致。

### 内訳は工場を守るツール
合計金額だけだと「車検が高い＝NG」と誤解される。内訳（予防交換・洗車サービス込み等）を見せると
「高いのではなく価値が入っている」と伝わる。→ 内訳表示は工場の価格を**正当化する営業ツール**。

### PR #50 の指標は安全
「今月の整備提案 件数・総額」は工場オーナーが**自店の実績を見るだけ**で、他店比較の公開ランキングではない。
値下げ競争の罠には該当しない。

### Phase 2 スコープ確定
「整備提案カードのUX強化」は **B面（私的履歴）限定のフル明細＋項目メモ**として実装する：
- 工場の作成フォーム：部品（番号/名称/メーカー/単価×数量）と作業（作業名/工賃）の行追加入力
- 各提案 or 各項目に「なぜ必要か / 含まれるサービス」の一言メモ（価格に文脈を同行させる）
- ユーザー側カード：合計＋**内訳を展開表示**（デフォルト畳む、タップで開く）
- 公開面には一切出さない
