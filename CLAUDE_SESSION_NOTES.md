...
**正となる作業ブランチ: `claude/continue-development-WYZZp`**

- 過去、AIが割り当てられた `claude/youthful-heisenberg-77st8f` で作業し続けた一方、
  ユーザーは `claude/continue-development-WYZZp` をローカルで動かしていたため、
  **改善が一切画面に反映されない**状態が長時間続いた（ダークテーマのまま／
  `scripts/seed_all.sh` が無い 等が証跡）。2026-06-18 に両ブランチを統合済み。
- **セッション開始時に必ず実行**:
  ```bash
  git branch --show-current        # ユーザーが動かすブランチと一致しているか
  git log --oneline -1             # 最新コミットが想定どおりか
  ```
  指定ブランチとユーザーのローカルブランチが食い違う場合は、**作業前に必ず確認**し、
  ユーザーが実際に動かすブランチへ統合（マージ）する。別ブランチへ push する際は許可を取る。
- ローカル起動はこのリポジトリに **hosting 設定が無い**＝自動公開版が無いため、
  ユーザーは必ず手元でビルドする。**ブランチ未pull＝旧UIのまま**になる点に常に注意。
