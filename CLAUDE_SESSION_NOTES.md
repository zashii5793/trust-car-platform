...
## 事業フォーカス戦略の確定（2026-08-20）

**ブランチ**: `claude/car-management-business-strategy-rgglyc` / **成果物**: `docs/BUSINESS_FOCUS_STRATEGY_2026-08.md`

外部から「機能が多すぎてターゲットが薄れる」との指摘。オーナーが挙げた12のメリットと
突き合わせて事業方針を確定した。

### 決定1: 機能は削らない。入口を1つにする

「機能が多すぎる」の正体は**入口が多すぎること**。既存機能の削除はコストだけ高く、
ユーザーが感じる混乱（ホームが複数ターゲットに同時に話しかけている）を解決しない。
→ **画面を3層化**（第1層=車検証OCR/期限/整備記録、第2層=記録2件以上で出現、
第3層=明示的に選択）＋**フリートは入口を分離**。削除ゼロ、露出制御のみ。

### 決定2: 売るのは「A分類」だけ。B/Cは約束しない

メリットを成立条件で分類:
- **A（自力で届く）**: 期限管理・整備記録・保険更新・法人車両管理 → **これだけを訴求**
- **B（相手の工場が要る）**: 問い合わせ・整備士連携・**査定に有利** → 相手が居るまで言わない
- **C（母数が要る）**: パーツ評価・ユーザー間のつながり → 母数が出るまで非表示

特に「査定に有利」は**評価する買取店が1社も無い＝約束できない**ため、初期文言から除外。

### 決定3: 開始点はタカヤの顧客一択。知人口コミは"次"

知人スタートは①課題が無い人が付き合いで入れる ②記録が発生しない ③本音のフィードバックが
出ない ④継続率が人間関係で歪み判断材料にならない、で危険。知人は**5〜10人のバグ出し要員まで**。
タカヤの顧客は「課題が実在」「裏書きする工場が既に居る」「入力ゼロにできる」「説明する人間が
居る」の4条件を満たす唯一の集団。かつ**個人・工場・法人フリートの3面すべての初期供給源**。
→ **1商圏で密度を作って鶏卵問題を割る**（Uber型）。90日は他商圏の営業を禁止。

### 決定4: 実装優先度は「新機能ゼロ」。既存を繋ぐ3件

最大の勝ち筋は「**整備完了時に工場が記録を手渡す**」体験（入力ゼロ＋`verifiedByShopId` 裏書き付き）。
1. **工場→ユーザーの記録引き渡し導線** — `InquiryMaintenancePayload` は実装済みだが
   **問い合わせスレッド内でしか発生しない**。普通に整備しただけの顧客に渡す導線が無い（最優先ギャップ）
2. **整備明細のUI表示** — `parts`/`unitPrice`/`nextReplacementDate` 等21項目がモデルのみでUI未接続
3. **次回交換予告→通知の接続** — このアプリの本体は画面ではなく**通知**。年5〜6回しか開かない以上、
   継続率を決めるのは**通知1通の的中率**

### 判定指標（DL数・登録数は見ない）

**「登録30日後に、工場が渡した記録とは別に自分で1件以上追加した人の割合」= 25%以上**が続行ライン。
他に90日継続（通知開封）50%以上、通知→来店/問い合わせ 90日で1件以上。
**この3つが出るまで、タカヤ以外の工場に営業に行かない。**
