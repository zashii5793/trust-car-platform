...
## コメント機能フォローアップ3件 ＋ 週末人間タスク整理（2026-08-12）

**ブランチ**: `claude/charming-shannon-ncqrvc` / **PR**: #66, #122（ともにスカッシュマージ済み）

### マージ済み
- #66: コメントのモデレーション自動非表示・ソート/ページング・通知
- #122: ①通報の**サーバー集計**（Cloud Functions `onCommentReportCreated`、クライアントの
  `reportCount`/`isHidden` 書込をルールで禁止） ②ページング/大量データ検証
  ③ソーシャル通知一覧＋ショーケース/投稿ディープリンク（SNSタブに未読バッジ付きベル）

### 決定事項
- コメントモデレーションの閾値は 3（`kReportHideThreshold` ⇔ functions の
  `REPORT_HIDE_THRESHOLD` を同期させること）
- ソーシャル通知の導線は「SNSタブの AppBar ベル」（既存通知タブはメンテ系のため分離）

### 人間タスク（今週末 2026-08-15/16 実施予定）
`docs/HUMAN_TASKS.md` 冒頭の「🗓 今週末の実施プラン」に集約（A〜E の順で実施）。
**特に P0-1（ルール）と P0-1.5（functions）はセット反映必須** — 片方だけだと通報集計が止まる。
