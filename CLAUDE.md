...
3. 作業開始をコメントで宣言する：
   ...
   gh project item-edit ... # ステータスをIn Progressに変更
   ```
...
Issueがない場合 → `/next` コマンドでカンバンから次タスクを提案する。
...
## GitHub Projects カンバン連携

**Project**: `github.com/users/zashii5793/projects/1`（Trust Car Platform）
**カラム構成**: Backlog → Ready → In Progress → Done
...
# カンバンのステータスをIn Progressに変更（item-idとfield-idは事前確認必要）
...
| コマンド | 用途 |
|---------|------|
| `/next` | カンバンを読んで次に着手すべきタスクを1件提案 |
| `/minutes <議事録テキスト>` | 議事録からアクションアイテムを抽出してIssue自動作成 |

### 議事録→Issue変換フロー

チャットに議事録テキストを貼り付けて `/minutes` を使うと:
1. アクションアイテムを自動抽出
2. `gh issue create` でIssue化
3. `gh project item-add` でBacklogに追加
